...
TASKS_DIR="/Users/h.grosse-plankermann/development/sideprojects/gsd/.claude/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/h.grosse-plankermann/development/sideprojects/gsd/.claude/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/h.grosse-plankermann/development/sideprojects/gsd/.claude/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
